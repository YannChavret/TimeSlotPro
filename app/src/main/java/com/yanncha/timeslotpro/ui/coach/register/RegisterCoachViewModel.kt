...
@HiltViewModel
class RegisterCoachViewModel @Inject constructor(
        ...
        if (validateInput(password, confirmPassword)) {
            viewModelScope.launch {
                withContext(Dispatchers.IO) {
                    if (coachDao.findCoachByIdentifiant(identifiant) != null) {
                        ...
                        Coach(
                            ...
                            coachDao.insertCoach(it)
                            _userMessageLiveData.postValue(R.string.registration_successful)
                            myPrefs.edit()?.apply {
                                putLong("id", coachDao.getCoachIdByLogin(identifiant)!!)
                                apply()
                            }
                        }
                    }
